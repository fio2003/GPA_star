...
def compute_on_local_machine(cpu_map: list, seed_list: list, cur_name: str, past_dir: str, work_dir: str, seed_dirs: dict,
    ...
    for i, exec_group in enumerate(cpu_map):
        ...
        for cur_group_sched in exec_group:
            ...
            # if xtc_filename in prev_runs_files and gro_filename in prev_runs_files:
...
def compute_with_mpi(seed_list: list, cur_name: str, past_dir: str, work_dir: str, seed_dirs: dict, topol_file_init: str,
    ...
    for i in range(tot_seeds):
        ...
        # if xtc_filename not in prev_runs_files or gro_filename not in prev_runs_files:
        if not (os.path.exists(os.path.join(past_dir, xtc_filename)) and os.path.exists(os.path.join(past_dir, gro_filename))): # \
                # and not (os.path.exists(os.path.join(extra_past, xtc_filename)) and os.path.exists(os.path.join(extra_past, gro_filename))):
...
def second_chance(open_queue: list, visited_queue: list, best_so_far_name: str, cur_metric: str, main_dict: dict,
    ...
    for elem in open_queue:
        if elem[2] == best_so_far_name[cur_metric]:
            recover_best = False
            break
    ...
    for elem in visited_queue:  # elem structure: tot_dist, att, cur_name
        ...
        if (elem[1] < node_max_att and main_dict[elem[2]]['{}_to_goal'.format(cur_metric_name)] - best_so_far[cur_metric] < tol_error[cur_metric_name]):  # \
...
def GMDA_main(prev_runs_files: list, past_dir: str, print_queue: mp.JoinableQueue,
    ...
    # cp2(os.path.join(prot_dir, 'md_heated.gro'), goal)

    # h_ndx_file = os.path.join(prot_dir, 'prot_h.ndx')
    ...
    gmx_trjconv(f=goal, o=goal_bb_xtc, n=goal_bb_ndx, s=goal)
    ...
    # In order to make plain you need three points, this is why you loos 2 elements. Last two do not have extra atoms to form a plain
    ...
    metric_names =      ['RMSD', 'ANGL', 'AND_H', 'AND', 'XOR']
    metric_allowed_sc = [  20,     10,      5,      5,    10  ]
    alowed_metrics =    ['RMSD', 'ANGL', 'AND_H', 'AND', 'XOR']
    cur_metric = 0
    cur_metric_name = alowed_metrics[cur_metric]
    ...
    err_mult = 0.8
    tol_error = check_precomputed_noize(an_file, metric_names)
    ...
    if tol_error is None or len(tol_error) < num_metrics:
        ...
        err_node_info = compute_init_metric(past_dir, tot_seeds, noize_file, goal_xtc_nz, goal_prot_only_nz, angl_num, goal_bb_ndx,
    ...
    print('Done measuring ambient noise for folded state at {}K.\n'
          ...
          ''.format(simulation_temp, tot_seeds, err_mult, tol_error['RMSD'], tol_error['ANGL'], tol_error['AND_H'],
                    tol_error['AND'], tol_error['XOR']))
    del err_mult
    node_info = compute_init_metric(past_dir, 1, init_xtc, goal_xtc, goal_prot_only, angl_num, init_bb_ndx, goal_angles, init_prot_only,
                                    ndx_file_init, goal_cont_h, atom_num, cont_dist, h_filter_init, goal_contacts,
                                    goal_contacts_and_h_sum, goal_contacts_and_sum)
    ...
    print('Unfolded to noise ratio:\n'
          ...
          'All contact disagreement (XOR): {:.5f}\n'.format(node_info['RMSD_to_goal'] / tol_error['RMSD'],
                                                            node_info['ANGL_to_goal'] / tol_error['ANGL'],
    ...
    open_queue = list()
    heapq.heappush(open_queue, (node_info['RMSD_to_goal'], 0, cur_hash_name))  # metric_val, attempts, name
    ...
    info_form_str = 'o_q:{:<5}  v_q:{:<3}  s:{:<3}  grm:{:6.3f}  gan:{:6.3f}  gah:{:<4}  gad:{:<4}  gxo:{:<4}  ' \
                    't:{:5.2f}s  gbr:{:.4f}  gba:{:.4f}  gc:{:<2}  ns:{:3.1f}  sc:{}'
    ...
    best_so_far = [node_info['{}_to_goal'.format(metr)] for metr in metric_names]
    print(best_so_far)
    best_so_far_name = [cur_hash_name] * num_metrics
    ...
    if recover:  # this can (and should) be done in parallel or instead of most var initialization (much earlier)
        ...
        best_so_far_name, best_so_far, greed_count = prev_state
        del prev_state
    ...
    while len(open_queue) > 0:  # and aa < 137:
        ...
        print_queue.put_nowait((info_form_str,
                                ((len(open_queue), len(visited_queue), skipped_counter, node_info['RMSD_to_goal'],
                                  node_info['ANGL_to_goal'], node_info['AND_H_to_goal'], node_info['AND_to_goal'],
                                  node_info['XOR_to_goal'], loop_end - loop_start, best_so_far[0], best_so_far[1],
                                  greed_count, greed_mult, seed_change_counter))))
        # print(info_form_str.format(len(open_queue), len(visited_queue), skipped_counter, node_info['RMSD_to_goal'],
        #                           node_info['ANGL_to_goal'], node_info['AND_H_to_goal'], node_info['AND_to_goal'],
        #                           node_info['XOR_to_goal'], loop_end - loop_start, best_so_far[0], best_so_far[1],
        #                           greed_count, greed_mult, seed_change_counter))
        ...
        # update prev files
        # prev_runs_files.extend(recent_filenames)
        if prev_runs_files:
            if len(prev_runs_files) <= tot_seeds*2:  # gro+xtc - two types
                prev_runs_files = None
            else:
                for file in recent_filenames:
                    try:
                        prev_runs_files.remove(file)
                    except Exception:
                        pass  # this is not an error - this behaviour is expected when you started following other route.
                        # print("Was not able to remove {}, list size: {}".format(file, len(prev_runs_files)))
                del file
        ...
        new_nodes, metric_to_goal, metric_form_prev, metric_to_tot = compute_metric(past_dir, new_nodes_names, tot_seeds, combined_pg,
                                                                                    temp_xtc_file, goal_prot_only, node_info, angl_num,
                                                                                    init_bb_ndx, goal_angles, init_prot_only,
                                                                                    ...
                                                                                    goal_contacts_and_sum, prev_runs_files is not None,
                                                                                    cpu_pool=cpu_pool,
        ...
        for i in range(tot_seeds):
            ...
            if (metric_form_prev[i] > tol_error[cur_metric_name]
                and metric_to_goal[i] - node_info['{}_to_goal'.format(cur_metric_name)] < tol_error[cur_metric_name] / 2) \
                    or metric_to_goal[i] <= best_so_far[cur_metric]:
        ...
        if compute_all_at_once:
            for metr in metric_names:
                if metr != cur_metric_name:
                    ...
                    if best_so_far[metric_names.index(metr)] > min_val:
                        # print('bsf["{}"]={:.4f}, min={:.4f}'.
                        # format(metr, best_so_far[metric_names.index(metr)], min_val), end=' ')
                        best_so_far[metric_names.index(metr)] = min_val
                    del min_val
                # else:
                    # print('skipping "{}"'.format(metr), end=' ')
            del metr
        # print()
        if best_so_far[guiding_metric] > new_nodes[metric_to_goal.index(min(metric_to_goal))]['{}_to_goal'.format(metric_names[guiding_metric])]:
            seed_change_counter = 0

        if best_so_far[cur_metric] > min(metric_to_goal):
            best_so_far_new = min(metric_to_goal)
            best_so_far[cur_metric] = best_so_far_new
            best_so_far_name[cur_metric] = new_nodes[metric_to_goal.index(best_so_far_new)]['digest_name']
            db_input_queue.put_nowait((insert_into_log,
                                       ('prom_O', best_so_far_name[cur_metric], '', '', best_so_far, greed_count, greed_mult,
                                        ...
                                        cur_metric_name)))
            if guiding_metric == cur_metric or best_so_far[guiding_metric] >= new_nodes[metric_to_goal.index(best_so_far_new)]['{}_to_goal'.format(metric_names[guiding_metric])]:
                for i in range(num_metrics):
                    if i != cur_metric:
                        best_so_far_name[i] = best_so_far_name[cur_metric]
            ...
            if greed_count in range(10, 101, 10):
                ...
                if seed_change_counter > metric_allowed_sc[cur_metric]:
                    new_metr_name = select_metrics_by_snr(new_nodes, node_info, metric_names, tol_error,
                                                          compute_all_at_once, alowed_metrics, cur_metric_name)
                ...
                #     seed_change_counter = 0
                #     greed_count = 112
                #     open_queue = proc_local_minim(open_queue, best_so_far_name[cur_metric], tol_error, ndx_file_init,
                #     name_2_digest_map, goal_prot_only, local_minim_names)
                #     local_minim_names = list()
                #     best_so_far[cur_metric] = (init_distance[cur_metric] + best_so_far[cur_metric])/2
